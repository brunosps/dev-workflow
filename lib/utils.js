...
function upsertDelimitedBlock(filePath, blockContent, startMarker, endMarker) {
  ...
  const blockStart = findStandaloneMarker(blockContent, startMarker);
  const blockEnd = blockStart
    ? findStandaloneMarker(blockContent, endMarker, blockStart.end)
    : null;
  ...
  const currentStart = findStandaloneMarker(current, startMarker);
  const currentEnd = currentStart
    ? findStandaloneMarker(current, endMarker, currentStart.end)
    : findStandaloneMarker(current, endMarker);
