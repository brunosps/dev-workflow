...
const { printPostUpdateActions } = require('./post-update');
...
async function run({ force = false, lang = null, mode = 'init', profile = null, modules = null }) {
  ...
  // 3.9. Copy curated rules library + stack mappings (declarative baseline per stack,
  // loaded lazily by the detected stack). Managed: reconciled on update/repair.
  const rulesLibSrc = path.join(SCAFFOLD_DIR, 'rules');
  if (require('fs').existsSync(rulesLibSrc)) {
    console.log('  Rules library:');
    const rlResults = copyDir(rulesLibSrc, path.join(projectRoot, '.dw', 'rules-library'), managedForce);
    totalCreated += rlResults.created;
    totalSkipped += rlResults.skipped;
    totalOverwritten += rlResults.overwritten;
    touchedFiles.push(...listFilesRecursive(path.join(projectRoot, '.dw', 'rules-library')));
    console.log(`    ${rlResults.created} created, ${rlResults.skipped} skipped, ${rlResults.overwritten} overwritten\n`);
  }
  const stackMapSrc = path.join(SCAFFOLD_DIR, 'config', 'stack-mappings.json');
  if (require('fs').existsSync(stackMapSrc)) {
    const cfgDir = path.join(projectRoot, '.dw', 'config');
    ensureDir(cfgDir);
    const stackMapDest = path.join(cfgDir, 'stack-mappings.json');
    const smStatus = writeFile(stackMapDest, require('fs').readFileSync(stackMapSrc, 'utf-8'), managedForce);
    if (smStatus === 'created') totalCreated++;
    else if (smStatus === 'overwritten') totalOverwritten++;
    else totalSkipped++;
    touchedFiles.push(stackMapDest);
  }

  // 3.95. Seed .dw/config/routing.json (dispatch tier/model/effort per brand for
  // the CLI runners). Seeded once and NEVER reconciled: model availability differs
  // per account, so the owner's edits must survive updates.
  const routingSrc = path.join(SCAFFOLD_DIR, 'config', 'routing.json');
  if (require('fs').existsSync(routingSrc)) {
    const cfgDir = path.join(projectRoot, '.dw', 'config');
    ensureDir(cfgDir);
    const routingDest = path.join(cfgDir, 'routing.json');
    // Refresh the candidate reference without replacing active owner choices.
    const defaultsDest = path.join(cfgDir, 'routing-defaults.json');
    const defaultsStatus = writeFile(defaultsDest, require('fs').readFileSync(routingSrc, 'utf-8'), managedForce);
    if (defaultsStatus === 'created') totalCreated++;
    else if (defaultsStatus === 'overwritten') totalOverwritten++;
    else totalSkipped++;
    touchedFiles.push(defaultsDest);
    const rtStatus = writeFile(routingDest, require('fs').readFileSync(routingSrc, 'utf-8'), false);
    if (rtStatus === 'created') totalCreated++;
    else totalSkipped++;
    touchedFiles.push(routingDest);
  }
  ...
  // 5.15. Seed .dw/minimalism.json (init only; preserved on update). Holds the
  // dw-minimalism intensity mode read by the skill and the statusline hook.
  if (!isUpdate) {
    const minimalismPath = path.join(projectRoot, '.dw', 'minimalism.json');
    const minimalismStatus = writeFile(minimalismPath, JSON.stringify({ mode: 'full' }, null, 2) + '\n', false);
    if (minimalismStatus === 'created') totalCreated++;
    else totalSkipped++;
    touchedFiles.push(minimalismPath);
  }
  ...
  // 5.25. Ensure .dw/metrics/ + .gitignore (runtime cost tracking; costs.jsonl is
  // machine-local so it is gitignored). Created if missing on both init and update.
  {
    const metricsDir = path.join(projectRoot, '.dw', 'metrics');
    ensureDir(metricsDir);
    const metricsGitignore = path.join(metricsDir, '.gitignore');
    const mStatus = writeFile(metricsGitignore, 'costs.jsonl\n', false);
    if (mStatus === 'created') totalCreated++;
    else totalSkipped++;
    touchedFiles.push(metricsGitignore);
  }

  // 5.25b. Ensure .dw/reports/ + .gitignore (progress-report loop; the daily logs and
  // the .active.json loop state are machine-local so they are gitignored, while other
  // reports such as context-budget.md stay tracked). Created if missing on init and update.
  {
    const reportsDir = path.join(projectRoot, '.dw', 'reports');
    ensureDir(reportsDir);
    const reportsGitignore = path.join(reportsDir, '.gitignore');
    const rStatus = writeFile(reportsGitignore, '????-??-??.md\n.active.json\n', false);
    if (rStatus === 'created') totalCreated++;
    else totalSkipped++;
    touchedFiles.push(reportsGitignore);
  }

  // 5.25c. Ensure .dw/cli-run/ + .gitignore (durable audit logs, session sidecars and
  // last-message captures of the dw-*-run adapters are machine-local; committing them
  // bloats every worktree checkout). Created if missing on init and update.
  {
    const cliRunDir = path.join(projectRoot, '.dw', 'cli-run');
    ensureDir(cliRunDir);
    const cliRunGitignore = path.join(cliRunDir, '.gitignore');
    const cStatus = writeFile(cliRunGitignore, '*\n!.gitignore\n', false);
    if (cStatus === 'created') totalCreated++;
    else totalSkipped++;
    touchedFiles.push(cliRunGitignore);
  }

  // 5.26. Ensure .dw/memory/instincts/ (durable learnings promoted by /dw-learn;
  // format in the dw-memory skill's references/instincts.md). Init only; preserved on update.
  if (!isUpdate) {
    const instinctsDir = path.join(projectRoot, '.dw', 'memory', 'instincts');
    ensureDir(instinctsDir);
    const instinctsKeep = path.join(instinctsDir, '.gitkeep');
    const ikStatus = writeFile(instinctsKeep, '', false);
    if (ikStatus === 'created') totalCreated++;
    else totalSkipped++;
    touchedFiles.push(instinctsKeep);
  }
  ...
  // 9.2. Reconcile dev-workflow hooks + statusline (marker-based, merge-aware:
  // refreshes our entries, never touches user-defined hooks/statusLine).
  console.log('  Hooks & statusline:');
  const hookResults = installHooks(projectRoot);
  console.log(`    git-guardrails: ${hookResults.hooks}, session-cost: ${hookResults.sessionCost}, statusline: ${hookResults.statusLine}\n`);
  if (hookResults.statusLine === 'skipped-user') {
    console.log('    Note: a custom statusLine is set; to show dev-workflow state add `node .dw/scripts/hooks/statusline.mjs`.\n');
  }
  ...
  console.log(`  Installed version: ${priorState?.version || 'unknown'} -> ${installState.version}\n`);
  ...
  console.log();
  if (isUpdate) {
    printPostUpdateActions(projectRoot);
  }
  console.log('  Next steps:');
  if (isUpdate) {
    console.log('    1. If running inside an agent, run the post-update actions now');
    console.log('    2. Otherwise reload/restart the agent session before invoking them');
