...
function renderClaudeAgent(agent, source) {
  ...
  const lines = [
    ...
    `tools: ${tools.join(', ')}`,
    // `inherit` is Claude Code's documented default: the subagent runs on the
    // session's model. Sizing happens at dispatch (the CLI runners pass an
    // explicit --model/--effort per .dw/config/routing.json), so pinning a tier
    // here would silently override the owner's session choice. A scaffold source
    // agent may still opt out by declaring its own `model:`.
    `model: ${source.metadata.model || 'inherit'}`,
