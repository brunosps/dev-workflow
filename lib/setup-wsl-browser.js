...
function run() {
  ...
  console.log(`  ${'='.repeat(40)}`);
  console.log('  \x1b[32m✓ Done.\x1b[0m The optional CDP fallback relay is installed in the Windows user profile.');
  console.log('    No admin prompt, firewall rule, or Windows Rust toolchain is required.');
  console.log(`    Set BROWSER_TEST to a Windows browser exe to make it available as fallback, e.g.`);
