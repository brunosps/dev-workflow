...
// Optional invocation control (mattpocock/skills). Bundled skills live under
// .agents/skills/ (read by path from command prose), so this is documental for
// them and feeds the à-la-carte plugin manifest. Default when absent: 'model'.
const VALID_INVOCATIONS = new Set(['model', 'explicit']);
...
function validateEntry(entry, issues) {
  ...
  // Optional fields (back-compat: absent is valid; defaults applied by consumers).
  if (entry.invocation !== undefined && !VALID_INVOCATIONS.has(entry.invocation)) {
    issues.push(`Skill registry entry ${entry.name} has invalid invocation: ${entry.invocation}`);
  }

  if (entry.exportable !== undefined && typeof entry.exportable !== 'boolean') {
    issues.push(`Skill registry entry ${entry.name} exportable must be a boolean`);
  }
}
