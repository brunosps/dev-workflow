...
const openDesign = require('./install-open-design');
...
function run() {
  ...
  const deps = [
    ...
    {
      name: 'Open Design CLI (od)',
      check: openDesign.check,
      install: openDesign.install,
      instructions: openDesign.instructions(),
      sync: openDesign.syncProjectAssets,
    },
  ];
  ...
  for (const dep of deps) {
    ...
    try {
      if (typeof dep.check === 'function' && dep.check()) {
        console.log(' \x1b[32m✓ already installed\x1b[0m');
        runDepSync(dep);
        installed++;
        continue;
      }
      if (typeof dep.install === 'function') {
        dep.install();
      } else {
...
function printBrowserGuidance() {
  ...
  console.log('    Optional CDP fallback: set BROWSER_TEST to a Windows browser exe path, e.g.');
  ...
  if (mirrored) {
    ...
    console.log('    NAT networking — run `npx @brunosps00/dev-workflow setup-wsl-browser` once to install the');
    console.log('    bundled user-level cdp-relay.exe. No admin prompt or firewall rule is required. Playwright');
    console.log('    Chromium remains the primary browser; CDP is only used if that launch fails.');
  }
...
function runDepSync(dep) {
  if (typeof dep.sync !== 'function') return;
  try {
    const result = dep.sync();
    if (result && result.synced === false && result.reason) {
      console.log(`    (project assets not synced: ${result.reason})`);
    }
  } catch (err) {
    console.log(`    (project asset sync failed: ${err.message})`);
  }
}
