...
const { validateSkillRegistry, readSkillRegistry, validateSkillStructuredReturn } = require('./skill-registry');
const { GIT_GUARDRAILS_CMD, STATUSLINE_CMD, HOOKS_DIR_MARKER } = require('./hooks');
...
function run() {
  ...
  checkHooks(projectRoot, issues, warnings);
...
function checkSkillRegistry(projectRoot, issues, warnings) {
  ...
  for (const entry of registry.skills) {
    ...
    if (!fs.existsSync(skillPath)) {
      ...
      validateSkillStructuredReturn(entry.name, skillPath, issues, `Installed bundled skill ${entry.name}`);
    }
  }
}
...
function checkHooks(projectRoot, issues, warnings) {
  // Hook scripts (copied from scaffold/scripts/ into .dw/scripts/).
  for (const script of ['git-guardrails.mjs', 'statusline.mjs', 'session-cost.mjs']) {
    const file = path.join(projectRoot, '.dw', 'scripts', 'hooks', script);
    if (!fs.existsSync(file)) issues.push(`Missing hook script: ${path.relative(projectRoot, file)}`);
  }

  const settingsPath = path.join(projectRoot, '.claude', 'settings.json');
  if (!fs.existsSync(settingsPath)) return; // checkMcp already reports this

  let settings;
  try {
    settings = JSON.parse(fs.readFileSync(settingsPath, 'utf-8'));
  } catch {
    return; // checkMcp already reports malformed settings
  }

  const preToolUse = (settings.hooks && settings.hooks.PreToolUse) || [];
  const guardrailWired = preToolUse.some(
    (g) => g && Array.isArray(g.hooks) && g.hooks.some((h) => h && h.command === GIT_GUARDRAILS_CMD),
  );
  if (!guardrailWired) issues.push('Missing git-guardrails PreToolUse hook in .claude/settings.json');

  const statusLine = settings.statusLine;
  if (!statusLine) {
    issues.push('Missing statusLine in .claude/settings.json');
  } else if (statusLine.command !== STATUSLINE_CMD && !String(statusLine.command || '').includes(HOOKS_DIR_MARKER)) {
    warnings.push('Custom statusLine set; dev-workflow statusline is not active (add `node .dw/scripts/hooks/statusline.mjs`).');
  }
}
