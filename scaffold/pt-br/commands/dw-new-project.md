...
2. Aplique o algoritmo de merge de `references/compose-composition.md`:
   ...
   - Quando pgvector for selecionado, preserve o digest da recipe, `POSTGRES_PASSWORD` obrigatoria e o bind de porta em `127.0.0.1`. Esse mapeamento loopback suporta intencionalmente `apps no host`; nao o remova do dev local.
...
- <critical>A imagem bundled `pgvector/pgvector:0.8.2-pg18` serve apenas para uma workstation local confiavel de um unico usuario. NUNCA a gere para producao, host de desenvolvimento remoto ou runner de CI compartilhado.</critical>
