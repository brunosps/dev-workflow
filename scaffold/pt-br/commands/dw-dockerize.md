...
Se pgvector for detectado ou adicionado, informe que a imagem bundled `pgvector/pgvector:0.8.2-pg18` e limitada a uma workstation local confiavel de um unico usuario. Nao a inclua para producao, host de desenvolvimento remoto ou runner de CI compartilhado. Em `--both`, o compose de dev local pode usar a recipe, mas o compose de producao deve usar imagem revisada separadamente ou servico gerenciado.
...
- Para pgvector, preserve o digest pinado, `POSTGRES_PASSWORD` obrigatoria e o bind em `127.0.0.1` da recipe. A porta loopback mantem intencionalmente a topologia `apps no host` funcionando.
...
- Nunca inclua `pgvector/pgvector:0.8.2-pg18`; use imagem revisada separadamente ou servico Postgres gerenciado.
...
- Compose: imagem bundled `pgvector/pgvector:0.8.2-pg18` fora de desenvolvimento local confiavel de um unico usuario? (flag CRITICAL)
...
- <critical>A imagem bundled `pgvector/pgvector:0.8.2-pg18` serve apenas para uma workstation local confiavel de um unico usuario. NUNCA use em producao, host de desenvolvimento remoto ou runner de CI compartilhado.</critical>
