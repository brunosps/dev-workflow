...
Se pgvector for detectado ou adicionado, informe que a imagem bundled `pgvector/pgvector:0.8.6-pg18-trixie` e limitada a uma workstation local confiavel de um unico usuario. Nao a inclua para producao, host de desenvolvimento remoto ou runner de CI compartilhado. Em `--both`, o compose de dev local pode usar a recipe, mas o compose de producao deve usar imagem revisada separadamente ou servico gerenciado.
...
- Para pgvector, preserve a tag da imagem da recipe (NAO converta para digest), `POSTGRES_PASSWORD` obrigatoria e o bind em `127.0.0.1` da recipe. A porta loopback mantem intencionalmente a topologia `apps no host` funcionando.
- Header: `# Generated by /dw-dockerize on YYYY-MM-DD`.

**`.env.example`** para pgvector: emita `POSTGRES_PASSWORD=` sem valor funcional. Depois da copia para `.env`, exija que o usuario defina uma senha local unica e gerada antes de rodar Compose. URLs das apps usam `localhost` quando apps rodam no host e DNS do servico quando rodam no Compose.
...
- Nunca inclua `pgvector/pgvector:0.8.6-pg18-trixie`; use imagem revisada separadamente ou servico Postgres gerenciado.
...
- Compose: imagem bundled `pgvector/pgvector:0.8.6-pg18-trixie` fora de desenvolvimento local confiavel de um unico usuario? (flag CRITICAL)
...
- <critical>A imagem bundled `pgvector/pgvector:0.8.6-pg18-trixie` serve apenas para uma workstation local confiavel de um unico usuario. NUNCA use em producao, host de desenvolvimento remoto ou runner de CI compartilhado.</critical>
