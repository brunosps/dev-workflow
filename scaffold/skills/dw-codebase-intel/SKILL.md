...
- Do not rebuild fresh intel unless requested; querying a fresh index remains appropriate.
- The project is greenfield with no source files yet — in that case, fall back to `.dw/rules/` seeded by `/dw-new-project`.

## Files in `.dw/intel/`

For files in `.dw/intel/`, read `references/files-in-dw-intel-detail.md`. Load only when this part of the task applies.

## Agents

For agents, read `references/agents-detail.md`. Load only when this part of the task applies.

## How to Compose (the typical flow)

For how to compose (the typical flow), read `references/how-to-compose-the-typical-flow-detail.md`. Load only when this part of the task applies.

## How `/dw-intel` Reads This

For how `/dw-intel` reads this, read `references/how-dw-intel-reads-this-detail.md`. Load only when this part of the task applies.
