...
- `/dw-refactor` -- reads rules for project context
...
<critical>PRESERVE the Grill flow's domain artifacts. `.dw/domain/**` (glossary / context-map / per-context files) is human-curated canonical vocabulary owned by `/dw-brainstorm` grill — this command must NEVER regenerate, merge, or overwrite it. When `.dw/domain/**` exists, READ it and LINK it from `.dw/rules/index.md`; do not copy its terms into the auto-generated `.dw/rules/` files. This command owns `.dw/rules/**`; the Grill flow owns `.dw/domain/**`.</critical>
...
- See [domain glossary](../domain/glossary.md) — or [context map](../domain/context-map.md) — for canonical vocabulary (only when `.dw/domain/` exists; owned by Grill, read-only here, never regenerated)
```

> When `.dw/domain/**` exists, add the Domain Vocabulary link(s) above to the generated `index.md` and leave the domain files untouched. When it does not exist, omit the link — this command never creates `.dw/domain/**` (only `/dw-brainstorm` grill does, after explicit authorization).
...
### Step 10: Design authority (DESIGN.md) — frontend projects only

Run this when a frontend was detected (React/Vue/Angular/Svelte/Tailwind/CSS) so `dw-ui-discipline`'s
grounding question 1 ("where do design decisions come from?") has a project authority to defer to. This
closes the loop the grounding only *reads* today.

1. **Check for an existing authority first.** If `DESIGN.md`, `BRAND.md`, or `STYLE_GUIDE.md` already
   exists at the project (or a frontend module) root, do NOT overwrite — note its location and stop.
2. **Detect design tokens** in priority order: Tailwind `theme`/`@theme` (colors, fontFamily, spacing,
   radius), CSS custom properties in `globals.css`/`theme.css`/`tokens.css`, MUI/Chakra theme exports,
   shadcn `components.json`, Storybook theme. Read the real values — do not invent.
3. **If tokens exist:** synthesize `DESIGN.md` at the frontend root capturing what the project ALREADY
   uses — palette (with the actual hex/oklch values and token names), typography (families, scale,
   weights), spacing/radius scale, and any component conventions observed. Head it with:
   `> Generated by /dw-analyze-project from existing tokens on <date>. Curate before treating as canon.`
4. **If no tokens exist:** do NOT fabricate a palette. Recommend bootstrapping from
   `dw-ui-discipline/references/curated-defaults.md` and record that recommendation in the rules index.
5. Reference the new `DESIGN.md` from `.dw/rules/<frontend-module>.md` so downstream commands find it.

This step is additive and reversible; it never edits component code, only documents what is there.
...
- [ ] Step 10 (design authority) ran for frontend projects: existing authority respected, or `DESIGN.md` generated from real tokens, or curated-defaults bootstrap recommended
- [ ] Preserved-marker blocks in concerns.md kept verbatim on refresh
- [ ] `.dw/domain/**` (glossary / context-map) preserved verbatim and linked from index.md when present — never regenerated or overwritten
