...
2. Apply the merge algorithm in `references/compose-composition.md`:
   ...
   - When pgvector is selected, preserve the recipe's digest, required `POSTGRES_PASSWORD`, and `127.0.0.1` port bind. That loopback mapping intentionally supports `apps on host`; do not remove it from local dev.
...
- <critical>The bundled `pgvector/pgvector:0.8.2-pg18` image is only for a trusted single-user local workstation. NEVER generate it for production, remote development hosts, or shared CI runners.</critical>
