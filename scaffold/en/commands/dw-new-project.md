...
6. **Database**: Postgres / Postgres + pgvector / MySQL / SQLite (file, no service) / MongoDB (out of scope for compose recipes — note and skip if chosen) / none. For Postgres + pgvector, the bundled recipe uses `pgvector/pgvector:0.8.6-pg18-trixie` and is allowed only on a trusted single-user local workstation. Do not offer it for production, remote development hosts, or shared CI runners.
...
1. Render `.dw/spec/projects/{{PROJECT_NAME}}.md` from `.dw/templates/project-onepager.md`. Fill every section: shape, languages, frameworks, services table (name + port + credential requirements, never a working password), architecture diagram (ASCII), generated-files list, open questions.
2. Build a plan:
...
2. Apply the merge algorithm in `references/compose-composition.md`:
   ...
   - When pgvector is selected, preserve the recipe's image tag (do NOT convert it to a digest), required `POSTGRES_PASSWORD`, and `127.0.0.1` port bind. That loopback mapping intentionally supports `apps on host`; do not remove it from local dev.
3. Add a header comment: `# Generated by /dw-new-project on YYYY-MM-DD`.

#### 3.4 Generate `.env.example`

Consolidate every env var referenced by selected services (per `references/env-conventions.md`). Group by service. Always include the application-side derived URLs (`DATABASE_URL`, `REDIS_URL`, `AMQP_URL`, `SMTP_HOST`/`SMTP_PORT`, `AWS_ENDPOINT_URL`, etc.). For pgvector, emit `POSTGRES_PASSWORD=` in `.env.example` and require the user to set a unique generated value in the gitignored `.env` before running Compose. Never emit a reusable password.
...
(table of selected services with ports and credential requirements; never include a working password)
...
- Quick Start (`cp .env.example .env`, set every required secret to a unique local value, then `pnpm install && pnpm dev:up`)
- Local Dev (port table for selected services + UI URLs + credential requirements; never publish a working password)
...
## Services Composed
<table of services with port + UI URL + credential requirements, sourced from .agents/skills/docker-compose-recipes/>

## Next Steps
1. `cp .env.example .env`, generate unique local secrets, and set every required value such as `POSTGRES_PASSWORD`.
...
- <critical>The bundled `pgvector/pgvector:0.8.6-pg18-trixie` image is only for a trusted single-user local workstation. NEVER generate it for production, remote development hosts, or shared CI runners.</critical>
