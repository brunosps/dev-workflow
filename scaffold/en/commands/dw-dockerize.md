...
If pgvector is detected or added, state that the bundled `pgvector/pgvector:0.8.2-pg18` image is limited to a trusted single-user local workstation. Do not include it for a production target, remote development host, or shared CI runner. In `--both`, the local dev compose may use the recipe, but the production compose must use a separately reviewed image or managed service.
...
- For pgvector, preserve the pinned digest, required `POSTGRES_PASSWORD`, and `127.0.0.1` bind from the recipe. The loopback port intentionally keeps the `apps on host` topology working.
...
- Never include `pgvector/pgvector:0.8.2-pg18`; use a separately reviewed image or managed Postgres service.
...
- Compose: bundled `pgvector/pgvector:0.8.2-pg18` image outside trusted single-user local development? (flag CRITICAL)
...
- <critical>The bundled `pgvector/pgvector:0.8.2-pg18` image is only for a trusted single-user local workstation. NEVER use it in production, on remote development hosts, or on shared CI runners.</critical>
