...
If pgvector is detected or added, state that the bundled `pgvector/pgvector:0.8.6-pg18-trixie` image is limited to a trusted single-user local workstation. Do not include it for a production target, remote development host, or shared CI runner. In `--both`, the local dev compose may use the recipe, but the production compose must use a separately reviewed image or managed service.
...
- For pgvector, preserve the recipe's image tag (do NOT convert it to a digest), required `POSTGRES_PASSWORD`, and `127.0.0.1` bind from the recipe. The loopback port intentionally keeps the `apps on host` topology working.
- Header comment: `# Generated by /dw-dockerize on YYYY-MM-DD`.

**`.env.example`** for pgvector: emit `POSTGRES_PASSWORD=` with no working value. After copying it to `.env`, require the user to set a unique generated local password before running Compose. Application URLs use `localhost` when apps run on the host and service DNS when they run in Compose.
...
**README.md "Local Dev" section** (appended): port table for selected services, credential requirements from `.env.example` without working passwords, the four `dev:*` script commands.
...
- Never include `pgvector/pgvector:0.8.6-pg18-trixie`; use a separately reviewed image or managed Postgres service.
...
- Compose: bundled `pgvector/pgvector:0.8.6-pg18-trixie` image outside trusted single-user local development? (flag CRITICAL)
...
8. **Next Steps:**
   - For `--dev`: `cp .env.example .env` (if missing), set every required secret such as `POSTGRES_PASSWORD` to a unique local value, run `docker compose -f docker-compose.dev.yml up -d`, then smoke test the app.
...
- <critical>The bundled `pgvector/pgvector:0.8.6-pg18-trixie` image is only for a trusted single-user local workstation. NEVER use it in production, on remote development hosts, or on shared CI runners.</critical>
