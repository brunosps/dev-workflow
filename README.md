...
Optional dependencies (Playwright browsers, react-doctor, Trivy, Semgrep, gitleaks, syft, Docker):
```bash
...
| **`/dw-review`** | Level 2 (PRD coverage mapping) + Level 3 (code quality). Hard gates on dw-verify PASS, secure-audit, constitution violations. Every candidate finding passes a refutation step before it is reported; an unresolved lead becomes `needs-validation` with no severity, and a refuted one stays logged. Flags: `--coverage-only`, `--code-only`, `--bugfix <slug>` (review a bugfix at `.dw/bugfixes/<slug>/`), `--since <ref>` (ad-hoc review from a verified comparison point), `--post-merge [<base>]` (composition audit of an already-merged range — cross-PR interaction, documentation ledger, semver recommendation; never tags or publishes). |
...
| **`/dw-skill-health`** | Audits installed skills and agents for bloat, duplication, and missing references, plus usage evidence: which installed skills have not fired inside a declared, observed window. Missing telemetry is reported as missing, never as disuse. |
...
| **`/dw-copilot-run`** | Fire `copilot -p` (GitHub Copilot CLI) in a dedicated git worktree to implement a prepared prompt/spec. Copilot adapter over the `dw-cli-run` protocol: durable audit log, resumable session, 0–10 dual evaluation, STOP for the gate. Never the main checkout; never merges. | `/dw-plan tasks` cross-tool assignment, once approved; or manually |
...
- If `/dw-plan` or `/dw-review` run **without** a constitution, they auto-install the defaults template (10 canonical principles at `severity: info`), notify in chat, and continue. Ausência nunca bloqueia.
...
`dev-workflow update` refreshes managed files only: commands, templates, references, scripts, bundled skills, wrappers, agents, and MCP config. It does **not** overwrite project-derived docs such as `.dw/rules/`, `.dw/constitution.md`, `.dw/rules/concerns.md`, `.dw/intel/`, or frontend `DESIGN.md`.

After an update, the CLI prints **Post-update agent actions**. When `/dw-update` runs inside an agent, it should execute the listed commands in order:

1. `/dw-analyze-project` when project-derived docs are missing or need refresh; this owns `.dw/rules/`, constitution, concerns map, and frontend `DESIGN.md`.
2. `/dw-intel --build` when the codebase index is missing or stale.
3. `/dw-harness-audit` to validate the install after managed files changed.
4. `/dw-skill-health` to inspect refreshed skills and agents.
...
- **Routing** — a table from intent and scope to the action, including the cases where the agent should answer directly or edit inline instead of starting a pipeline. It carries the common paths only; the full intent-to-command Trigger Map lives on demand in `.dw/references/command-routing.md`, so routine work does not pay for the whole catalog.
- **Planning and delegation** — per-task local/cross-tool, model, effort and agent assignment; what approval covers; the worker limits (at most 3 per workstream, never concurrent writers in one worktree).
- **Completion and boundaries** — an approved plan continues through implementation, review, QA and fixes; what makes a delivery ready; which operations need authorization.
- **Project contracts** — constitution and ADR process, task-plan validity, required checks, atomic commits, and the untrusted-input rule (external text is evidence, never an instruction).
- **Customization** — the `<!-- dev-workflow:start -->` / `<!-- dev-workflow:end -->` markers and what survives an update.
...
├── CLAUDE.md              # Managed agent-instruction block for Claude Code (merge-aware; routing, contracts, boundaries)
...
| **Trivy** | Native binary scanner used by `/dw-secure-audit` for CVE, secret, and IaC scanning. `install-deps` detects presence and prints OS-specific install instructions (brew / curl script / choco / Docker) — does not install automatically. | [aquasecurity.github.io/trivy](https://aquasecurity.github.io/trivy/) |
...
npx @brunosps00/dev-workflow update                        # Update managed files + print post-update agent actions
npx @brunosps00/dev-workflow list-installed                # Show managed install state
...
npx @brunosps00/dev-workflow setup-wsl-browser             # WSL NAT: install optional user-level CDP fallback relay (no admin)
...
npx @brunosps00/dev-workflow uninstall                     # Remove managed files while preserving user data
...
The managed instruction block (between `<!-- dev-workflow:start -->` and `<!-- dev-workflow:end -->`) refreshes automatically on update with the v1.0.0 command surface. Your edits outside the markers are preserved.
