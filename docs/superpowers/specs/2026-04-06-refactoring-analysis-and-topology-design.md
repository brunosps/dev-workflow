# Design: /refactoring-analysis Command + Topology Analysis in /analyze-project
...
The dev-workflow pipeline has a gap between `/analyze-project` (maps the project) and `/code-review` (reviews changes before PR). There is no command focused on identifying refactoring opportunities in existing code using a structured methodology.

Additionally, the `/analyze-project` command lacks topological analysis of code dependencies — god nodes, coupling metrics, and dependency graphs that tools like graphify provide.

This spec covers:
1. A new `/refactoring-analysis` command (EN + PT-BR)
2. A topology analysis section added to `/analyze-project` (EN + PT-BR)

## Part 1: /refactoring-analysis Command
...
- `scaffold/en/commands/refactoring-analysis.md`
...
- `ai/spec/prd-[feature]/refactoring-analysis.md`

### Pipeline Position

Can be executed at any time, but recommended:
- Before starting a feature (understand tech debt in the area)
- After `/review-implementation` (identify post-feature refactoring opportunities)
...
- Read `ai/rules/` for project context, architecture, and conventions
...
## Part 2: Topology Analysis in /analyze-project
...
- `scaffold/en/commands/analyze-project.md` — add Step 4.1 + output section
- `scaffold/pt-br/commands/analyze-project.md` — add Step 4.1 + output section
...
- `README.md` — add `/refactoring-analysis` to the Commands section under Quality

### Verification

After implementation:
1. Run `npx dev-workflow init --lang=en --force` in a test directory and verify `refactoring-analysis.md` appears in `ai/commands/`
